...
* [ ] read the separate .csv-files (`Parameters`):
	...
	* [ ] identify the type of stored parameters and convert them to the other sets of parameters. Store these in new files in the same folder with keyword `AllParameters`.

	* [ ] use ChemicalIdentifiers.jl to look up substances

	* [ ] build from all extracted informations databases (new and old format) for every .csv-file separtly AND one combined
